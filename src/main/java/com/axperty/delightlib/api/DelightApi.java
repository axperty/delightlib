...
public interface DelightApi {
    /**
     * Creates a new instance of the Delight API for your mod. You should call this method in your mod's constructor or common setup method to initialize the API and start registering your content. The mod ID you provide will be used as a namespace for all registry entries created through this API, so make sure it is unique and does not conflict with other mods. The event bus you provide will be used to register any necessary event listeners for the content you create with this API, so it should be the mod event bus that you use for your mod's registration.
     *
     * @param modId       the mod ID to use as a namespace for all registry entries created through this API. This should be the same mod ID that you use in your mod's @Mod annotation and build configuration, and it should be unique to avoid conflicts with other mods.
     * @param modEventBus the mod event bus to register any necessary event listeners for the content created with this API. This should be the same event bus that you use for your mod's registration, and it is typically passed to your mod's constructor by the mod loading system.
     ...
     */
    static DelightApi create(String modId, IEventBus modEventBus) {
        return DelightAddon.internalCreate(modId, modEventBus);
    }
